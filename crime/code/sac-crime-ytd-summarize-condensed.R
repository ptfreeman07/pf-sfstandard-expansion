...
city.crimes.type <- crime_grid %>% 
  group_by(month, standard.crime.type.super.label) %>%
  dplyr::summarise(total.of.type = sum(count.in.month)) %>% 
  dplyr::filter(!is.na(month)) %>%
  ungroup() %>%
  pivot_wider(., names_from="standard.crime.type.super.label", values_from="total.of.type", values_fill=0) %>%
  clean_names()
...
### Write to csv
write_csv(all.summary.statements, "outputs/sac-neighborhood-crime-summary-2022-07-16_condensed.csv")
